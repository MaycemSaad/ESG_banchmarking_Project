...
# ------------------ LOAD KPI DATABASE ------------------
try:
    kpi_df = pd.read_csv('esg kpis A+ critical(Sheet1).csv', encoding='ISO-8859-1', sep=';')
except Exception as e:
    logging.error(f"Error loading KPI reference CSV: {e}")
    kpi_df = pd.DataFrame([])
...
app.layout = dbc.Container([
    dcc.Store(id='kpi-database', data=kpi_df.to_dict('records')),

    html.H1("🌱 ESG KPI Extraction Tool", className="mt-4 mb-4 text-center"),
    html.P("Upload sustainability reports to extract critical ESG performance indicators", className="text-muted text-center mb-4"),
    ...
    dbc.Row([
        dbc.Col([html.Div(id='file-list', className="mt-3")], width=6),
        dbc.Col([
            dbc.Card([
...
return no_update, no_update, table
    ...
    for content, filename in zip(contents, filenames):
        try:
            ...
            tabs = dbc.Tabs([
                ...
                dbc.Tab(
                    html.Pre(json_str, style={'height': '400px', 'overflowY': 'scroll'}),
                    label="Raw JSON"
                )
            ])
            ...
            download_btn = dbc.Button(
                ...
                color="success",
                className="mt-3",
                href=f"data:application/json;base64,{encoded_json}",
                download=f"{filename}_esg_kpis.json"
            )
