...
# Fonction robuste pour lire les CSV
def safe_read_csv(file_path):
    ...
    try:
        ...
        try:
            # If normal read fails, try with error_bad_lines=False (older pandas)
...
def find_relevant_kpis(text, kpi_embeddings, all_kpis, threshold=0.55):
    ...
    # Diviser le texte en chunks pour l'analysis
...
def show_statistics():
    if os.path.exists(OUTPUT_CSV):
        ...
        if not df.empty:
            ...
            print("Fichier CSV vide ou corrompu.")
    else:
...
# Load dataset for dashboard & analysis with robust error handling
if os.path.exists(OUTPUT_CSV):
...
if show_stats_button:
    ...
    if not df.empty:
        ...
        col2.metric("Companies", df['source_file'].nunique())
        col3.metric("Unique Topics", df['topic'].nunique() if 'topic' in df.columns else 0)
        if 'extraction_date' in df.columns:
            last_extraction = df['extraction_date'].max()
        else:
            last_extraction = "Unknown"
        col4.metric("Last Extraction", last_extraction)
        
        st.markdown("#### Processed Files:")
        for file in df['source_file'].unique():
            count = len(df[df['source_file'] == file])
            st.write(f"- **{file}**: {count} KPIs")
    else:
