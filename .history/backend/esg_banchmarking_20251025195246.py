...
def find_relevant_kpis(text, kpi_embeddings, all_kpis, threshold=0.4):  # Seuil réduit
    ...
    for chunk_idx, chunk in enumerate(chunks):
        ...
        try:
            if len(chunk) > 10000:
                ...
                sentences = re.split(r'[.!?]+', chunk)
                sentences = [s.strip() for s in sentences if len(s.strip()) > 20]  # Longueur réduite
            else:
        ...
        for i, sentence in enumerate(sentences):
            ...
            try:
                ...
                top_results = np.argsort(-cos_scores.cpu().numpy())[:5]  # Top 5 au lieu de 3
...
# Vérifier la cohérence des valeurs - ASSOUPLI
...
# Extraire les valeurs numériques - AMÉLIORÉ
...
def filter_results(results, min_confidence=0.3):  # Seuil réduit
    filtered = []
    seen = set()
...
# Traiter un PDF - AMÉLIORÉ avec plus de logging
def process_pdf(pdf_path, kpi_embeddings, all_kpis, kpi_df, min_confidence=0.3):  # Seuil réduit
...
def process_pdf_route():
    try:
        ...
        min_confidence = float(request.form.get('min_confidence', 0.3))  # Default réduit
