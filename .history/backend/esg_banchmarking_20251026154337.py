...
Contexte: {context_data[:1000]}  # Limiter la taille du contexte
Réponds en français de façon concise."""
        ...
        payload = {
            ...
            "options": {
                ...
                "top_p": 0.9,
                "top_k": 40,
                "num_predict": 300,  # Réponse plus courte
                "num_ctx": 2048      # Contexte réduit
            }
        }
        
        # Timeout plus long mais raisonnable
        response = requests.post(
            f"{OLLAMA_BASE_URL}/api/generate",
            json=payload,
            timeout=60  # 60 secondes devraient suffire
        )
        ...
        logger.error("Timeout Ollama après 60 secondes")
...
def prepare_chatbot_context():
    """Préparer un contexte optimisé pour le chatbot"""
    try:
        ...
        # Contexte très court et optimisé
        context_parts = []
        
        # Informations essentielles seulement
        context_parts.append(f"KPIs: {len(df)}")
        context_parts.append(f"Entreprises: {df['source_file'].nunique()}")
        
        # Top 3 KPIs seulement
        if not df.empty:
            top_kpis = df['kpi_name'].value_counts().head(3)
            context_parts.append("Top KPIs:")
            for kpi, count in top_kpis.items():
                context_parts.append(f"- {kpi}: {count}")
        
        return " | ".join(context_parts)  # Format compact
...
def chatbot_chat():
    ...
    try:
        ...
        logger.info(f"Contexte préparé: {len(context_data)} caractères")
