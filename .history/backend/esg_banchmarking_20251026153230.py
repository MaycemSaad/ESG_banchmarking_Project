...
def query_ollama(prompt, context_data=""):
    """Interroger le modèle Ollama avec gestion robuste des erreurs"""
    try:
        # Vérifier d'abord si Ollama est accessible
        if not check_ollama_connection():
            return "❌ Ollama n'est pas accessible. Veuillez démarrer Ollama avec la commande 'ollama serve' et vérifier qu'il écoute sur le port 11434."
        ...
        Soyez précis et utilisez les données disponibles.
        """
        ...
        payload = {
            ...
            "options": {
                ...
                "top_p": 0.9,
                "top_k": 40,
                "num_predict": 1000  # Limiter la longueur de la réponse
            }
        }
        ...
        response = requests.post(
            ...
            json=payload,
            timeout=30  # Réduit de 120 à 30 secondes
        )
        ...
        if response.status_code == 200:
            ...
            return f"Erreur de communication avec Ollama (HTTP {response.status_code}). Veuillez réessayer."
            
    except requests.exceptions.Timeout:
        logger.error("Timeout lors de l'appel à Ollama")
        return "⏰ Délai d'attente dépassé. Ollama met trop de temps à répondre. Veuillez réessayer avec une question plus simple ou vérifier que le modèle est correctement chargé."
    
    except requests.exceptions.ConnectionError:
        logger.error("Erreur de connexion à Ollama")
        return "🔌 Erreur de connexion à Ollama. Vérifiez que Ollama est démarré avec 'ollama serve' et qu'il écoute sur le port 11434."
...
def prepare_chatbot_context():
    ...
    try:
        ...
        context_parts.append(f"Entreprises analysées: {', '.join(df['source_file'].unique()[:5])}")  # Limiter à 5 entreprises
        ...
        if not df.empty:
            ...
            # Prendre des KPIs avec une bonne confiance
            high_confidence_kpis = df[df['confidence'] > 0.5].head(3) if 'confidence' in df.columns else df.head(3)
            sample_kpis = high_confidence_kpis[['kpi_name', 'value', 'unit']].to_dict('records')
