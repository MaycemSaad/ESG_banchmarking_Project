...
def load_kpi_list(file_path):
    ...
    # Trouver les colonnes KPI avec plus de flexibilité
    kpi_name_col = None
...
# Trouver les KPIs pertinents - AMÉLIORÉ
def find_relevant_kpis(text, kpi_embeddings, all_kpis, threshold=0.4):
    ...
    for chunk_idx, chunk in enumerate(chunks):
        ...
        try:
            if len(chunk) > 10000:
                # Segmentation simple pour les gros chunks
                sentences = re.split(r'[.!?]+', chunk)
                sentences = [s.strip() for s in sentences if len(s.strip()) > 20]
            else:
                # Utiliser spaCy pour une meilleure segmentation
                doc = nlp(chunk)
        ...
        for i, sentence in enumerate(sentences):
            ...
            try:
                ...
                top_results = np.argsort(-cos_scores.cpu().numpy())[:3]  # Top 3 seulement
                
                for idx in top_results:
                    if cos_scores[idx] > threshold:
                        kpi_name = all_kpis[idx]
                        # Éviter les doublons dans le même chunk
...
# Extraire les valeurs numériques - AMÉLIORÉ avec gestion des doublons
def extract_kpi_values(text, kpi_name):
    ...
    values = []
    seen_values = set()  # Pour éviter les doublons
    ...
    for pattern in patterns:
        ...
        for match in matches:
            ...
            try:
                # Gérer les millions, billions, etc.
                multiplier = 1
...
# Filtrer les résultats - AMÉLIORÉ avec meilleure déduplication
def filter_results(results, min_confidence=0.3):
    filtered = []
    seen = set()
    
    # Trier par confiance (meilleure confiance en premier)
    results_sorted = sorted(results, key=lambda x: x['confidence'], reverse=True)
    
    for result in results_sorted:
        # Créer un identifiant plus strict pour éviter les doublons
        identifier = f"{result['kpi_name']}_{result['value']}_{result['unit']}_{result['source_file']}"
        
        # Vérifier si on a déjà un KPI similaire
        if (identifier not in seen and 
            result['confidence'] >= min_confidence):
            filtered.append(result)
            seen.add(identifier)
    
    # Grouper par KPI et ne garder que la meilleure occurrence
    ...
    for kpi_name, occurrences in kpi_groups.items():
        # Prendre l'occurrence avec la meilleure confiance
        best_occurrence = max(occurrences, key=lambda x: x['confidence'])
        final_results.append(best_occurrence)
    
    return final_results

# Traiter un PDF - AMÉLIORÉ avec meilleure gestion des doublons
def process_pdf(pdf_path, kpi_embeddings, all_kpis, kpi_df, min_confidence=0.3):
    ...
    with open(debug_file, 'w', encoding='utf-8') as f:
        f.write(text[:5000] + "\n[...]")  # Sauvegarder les premiers 5000 caractères
    
    print(f"Texte debug sauvegardé: {debug_file}")
    
    # Trouver les KPIs pertinents avec seuil réduit
    ...
    for kpi_name, matches in relevant_kpis.items():
        ...
        # Trier les matches par score (meilleur en premier)
        matches_sorted = sorted(matches, key=lambda x: x['score'], reverse=True)
        
        # Prendre seulement les 3 meilleures correspondances pour ce KPI
        for match in matches_sorted[:3]:
            ...
            # Pour chaque valeur extraite, créer un résultat
            for val in values:
                # Trouver le KPI original dans le DataFrame
                kpi_info = kpi_df[
                    (kpi_df.iloc[:, 0] == kpi_name) | 
                    (kpi_df.iloc[:, 1] == kpi_name)
                ]
    ...
    # Filtrer les résultats avec déduplication améliorée
